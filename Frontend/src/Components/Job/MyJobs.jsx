...
const MyJobs = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const { data } = await axios.get("http://localhost:2321/api/v1/job/getOwnJobs",
  ...
  const handleEnableEdit = async (jobId) => {
  ...
  const handleUpdateJob = async (jobId) => {
    ...
    await axios.put(`http://localhost:2321/api/v1/job/updateJobs/${jobId}`, updatedJob, {
      withCredentials: true
    }).then((res) => {
  ...
  const handleDeleteJob = async (jobId) => {
    await axios.delete(`http://localhost:2321/api/v1/job/deleteJobs/${jobId}`,
      {
        withCredentials: true
      }).then((res) => {
        toast.success(res.data.message)
      ...
      <div className='container'>
        <h3>YOUR POSTED JOBS</h3>
        {
         myJobs && myJobs.length > 0 ? (
            ...
            : (<p>You have not posted any job or may be deleted all of your jobs! </p>)
        }
      </div>
    </div>
  )
